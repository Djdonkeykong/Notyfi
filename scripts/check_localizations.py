...
def compare_locales(
    ...
    for locale, entries in sorted(locales.items()):
        ...
        for key in missing_keys:
            errors.append(f"{label}: locale '{locale}' is missing key '{key}'.")
        for key in extra_keys:
            errors.append(f"{label}: locale '{locale}' has extra key '{key}' not present in English.")
...
def main() -> int:
    ...
    errors.extend(compare_locales("App localizations", app_locales, "en", app_referenced_keys))
    errors.extend(compare_locales("Widget localizations", widget_locales, "en", widget_referenced_keys))
